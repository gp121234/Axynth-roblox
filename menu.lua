...
sep(tF2)
lbl(tF2,">> SELF EFFECTS")
btn(tF2,"Fire On Self",function() pcall(function() local ch=LP.Character if ch then local hrp=ch:FindFirstChild("HumanoidRootPart") if hrp then local fire=Instance.new("Fire") fire.Size=5 fire.Heat=5 fire.Parent=hrp end end end) end,"selffire")
btn(tF2,"Remove Fire",function() pcall(function() local ch=LP.Character if ch then local hrp=ch:FindFirstChild("HumanoidRootPart") if hrp then for _,v in pairs(hrp:GetChildren()) do if v:IsA("Fire") then v:Destroy() end end end end end) end,"rmselffire")
btn(tF2,"Sparkles On Self",function() pcall(function() local ch=LP.Character if ch then local hrp=ch:FindFirstChild("HumanoidRootPart") if hrp then local sp=Instance.new("Sparkles") sp.Parent=hrp end end end) end,"selfspark")
btn(tF2,"Smoke On Self",function() pcall(function() local ch=LP.Character if ch then local hrp=ch:FindFirstChild("HumanoidRootPart") if hrp then local sm=Instance.new("Smoke") sm.Size=3 sm.Opacity=0.5 sm.Parent=hrp end end end) end,"selfsmoke")
btn(tF2,"Remove All FX",function() pcall(function() local ch=LP.Character if ch then local hrp=ch:FindFirstChild("HumanoidRootPart") if hrp then for _,v in pairs(hrp:GetChildren()) do if v:IsA("Fire") or v:IsA("Sparkles") or v:IsA("Smoke") then v:Destroy() end end end end end) end,"rmsselffx")
sep(tF2)
