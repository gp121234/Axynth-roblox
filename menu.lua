                        ...
                        if bestTarget and bestTarget.Character and bestTarget.Character:FindFirstChild(ST.aimTargetPart) then
                            ...
                            for i=1,#args do
                                ...
                                if typeof(v)=="Vector3" then
                                    ...
                                    local mag=(tgtPos-camPos).Magnitude
                                    if mag>5 and mag<500 then args[i]=dir end
...
local tSPH=tog(tH,"Speed Hard",function() return ST.speedHard end,function() ST.speedHard=not ST.speedHard ntf("SpeedHard",ST.speedHard and "ON (70)" or "OFF") end,"speedhard")
...
local tFC=tog(tH,"Free Cam",function() return ST.freeCam end,function() ST.freeCam=not ST.freeCam if ST.freeCam then ST.freeCamPos=CAM.CFrame ST.freeCamYaw=0 ST.freeCamPitch=0 CAM.CameraType=Enum.CameraType.Scriptable if LP.Character then local h=LP.Character:FindFirstChildOfClass("Humanoid") if h then h.PlatformStand=true CAM.CameraSubject=h end end ntf("FreeCam","ON - WASD + Mouse") else CAM.CameraType=Enum.CameraType.Custom if LP.Character then local h=LP.Character:FindFirstChildOfClass("Humanoid") if h then h.PlatformStand=false CAM.CameraSubject=h end end ntf("FreeCam","OFF") end end,"freecam")
...
sep(tW)
lbl(tW,">> GRAVITY")
btn(tW,"Gravity +50",function() W.Gravity=W.Gravity+50 ntf("Gravity",math.floor(W.Gravity)) end,"gravup")
btn(tW,"Gravity -50",function() W.Gravity=math.max(0,W.Gravity-50) ntf("Gravity",math.floor(W.Gravity)) end,"gravdn")
btn(tW,"Gravity: Moon",function() W.Gravity=13 ntf("Gravity","Moon (13)") end,"gravmoon")
btn(tW,"Gravity: Normal",function() W.Gravity=196.2 ntf("Gravity","Normal (196.2)") end,"gravnorm")
btn(tW,"Gravity: Zero",function() W.Gravity=0 ntf("Gravity","Zero-G (0)") end,"gravzero")
...
btn(tP,"Add Marker Here",function() pcall(function() if LP.Character and LP.Character:FindFirstChild("HumanoidRootPart") then local pos=LP.Character.HumanoidRootPart.Position table.insert(ST.cycleMarkers,{pos=pos}) local p=Instance.new("Part") p.Name="AxCycleMk"..#ST.cycleMarkers p.Size=Vector3.new(2,0.2,2) p.Anchored=true p.CanCollide=false p.Material=Enum.Material.Neon p.Color=Color3.fromRGB(255,50,50) p.Transparency=0.3 p.Position=pos-Vector3.new(0,3,0) p.Parent=W local bb=Instance.new("BillboardGui") bb.Size=UDim2.new(0,50,0,30) bb.StudsOffset=Vector3.new(0,2,0) bb.AlwaysOnTop=true bb.Parent=p local tl=Instance.new("TextLabel") tl.Size=UDim2.new(1,0,1,0) tl.BackgroundTransparency=1 tl.Text="#"..#ST.cycleMarkers tl.TextColor3=Color3.new(1,1,1) tl.TextSize=14 tl.Font=Enum.Font.GothamBold tl.Parent=bb ST.cycleMarkers[#ST.cycleMarkers].part=p ntf("CycleTP","Marker #"..#ST.cycleMarkers.." added!") end end) end,"addcmark")
btn(tP,"Remove Last Marker",function() if #ST.cycleMarkers>0 then local m=table.remove(ST.cycleMarkers) if m.part and m.part.Parent then m.part:Destroy() end ntf("CycleTP","Removed. Left: "..#ST.cycleMarkers) end end,"rmcmark")
...
sep(tEx)
lbl(tEx,">> ATM / MONEY")
btn(tEx,"Spam ATM x10",function() if cd() then pcall(function() local r=findRemote("ATM") if r then for i=1,10 do r:FireServer() task.wait(0.1) end ntf("ATM","Fired x10!") end end) end end,"spamatm10")
btn(tEx,"Spam ATM x50",function() if cd() then pcall(function() local r=findRemote("ATM") if r then for i=1,50 do r:FireServer() task.wait(0.05) end ntf("ATM","Fired x50!") end end) end end,"spamatm50")
btn(tEx,"Spam ATM x100",function() if cd() then pcall(function() local r=findRemote("ATM") if r then for i=1,100 do r:FireServer() task.wait(0.02) end ntf("ATM","Fired x100!") end end) end end,"spamatm100")
sep(tEx)
lbl(tEx,">> EFOOD JOB (MONEY)")
btn(tEx,"Accept Efood x10",function() if cd() then pcall(function() local r=findRemote("efood.acceptOrder") if r then for i=1,10 do r:FireServer() task.wait(0.1) end ntf("Efood","Accepted x10!") end end) end end,"efoodacc10")
btn(tEx,"Accept Efood x50",function() if cd() then pcall(function() local r=findRemote("efood.acceptOrder") if r then for i=1,50 do r:FireServer() task.wait(0.05) end ntf("Efood","Accepted x50!") end end) end end,"efoodacc50")
btn(tEx,"Deliver Efood x50",function() if cd() then pcall(function() local r=findRemote("efood.deliveredOrder") if r then for i=1,50 do r:FireServer() task.wait(0.05) end ntf("Efood","Delivered x50!") end end) end end,"efoodeliv50")
btn(tEx,"Start Efood Job",function() if cd() then pcall(function() local r=findRemote("efood.efoodJob") if r then r:FireServer() ntf("Efood","Job started!") end end) end end,"efoodjob")
sep(tEx)
lbl(tEx,">> BANK ROBBERY")
btn(tEx,"Spam Bank Rob x20",function() if cd() then pcall(function() local r=findRemote("Bank.TablesEvents.Prompt") if r then for i=1,20 do r:FireServer() task.wait(0.1) end ntf("Bank","Fired x20!") end end) end end,"bankrob20")
btn(tEx,"Spam Vault Lock x20",function() if cd() then pcall(function() local r=findRemote("Bank.Lockpick.Vault") if r then for i=1,20 do r:FireServer() task.wait(0.1) end ntf("Lockpick","Vault x20!") end end) end end,"lockvault20")
btn(tEx,"Spam Lock x20",function() if cd() then pcall(function() local r=findRemote("Bank.Lockpick.Lock") if r then for i=1,20 do r:FireServer() task.wait(0.1) end ntf("Lockpick","Lock x20!") end end) end end,"locklock20")
btn(tEx,"Spam Grab Cash x20",function() if cd() then pcall(function() local r=findRemote("Bank.TablesEvents.Grab") if r then for i=1,20 do r:FireServer() task.wait(0.1) end ntf("Bank","Grabbed x20!") end end) end end,"bankgrab20")
sep(tEx)
lbl(tEx,">> THIEF + SOCIETY")
btn(tEx,"Spam Thief x20",function() if cd() then pcall(function() local r=findRemote("ThiefSystem.RemoteEvent") if r then for i=1,20 do r:FireServer() task.wait(0.1) end ntf("Thief","Fired x20!") end end) end end,"thief20")
btn(tEx,"Spam Society x20",function() if cd() then pcall(function() local r=findRemote("Society.MainEvent") if r then for i=1,20 do r:FireServer() task.wait(0.1) end ntf("Society","Fired x20!") end end) end end,"soc20")
btn(tEx,"Spam Inventory x50",function() if cd() then pcall(function() local r=findRemote("Inventory.Inventory") if r then for i=1,50 do r:FireServer() task.wait(0.05) end ntf("Inventory","Fired x50!") end end) end end,"spaminv50")
sep(tEx)
lbl(tEx,">> DJ + SIREN")
btn(tEx,"Open DJ",function() pcall(function() local r=findRemote("Dj System.OpenDj") if r then r:FireServer() ntf("DJ","Opened!") end end) end,"opendj")
btn(tEx,"DJ Vol Up x10",function() if cd() then pcall(function() local r=findRemote("Dj System.IncreaseMusicVolume") if r then for i=1,10 do r:FireServer() task.wait(0.05) end ntf("DJ","Up x10!") end end) end end,"djup10")
btn(tEx,"DJ Vol Down x10",function() if cd() then pcall(function() local r=findRemote("Dj System.DecreaseMusicVolume") if r then for i=1,10 do r:FireServer() task.wait(0.05) end ntf("DJ","Down x10!") end end) end end,"djdn10")
btn(tEx,"Toggle DJ Sound",function() pcall(function() local r=findRemote("Dj System.ToggleSound") if r then r:FireServer() ntf("DJ","Toggled!") end end) end,"djtoggle")
sep(tEx)
sep(tEx)
...
local tVS=tog(tMi,"Vehicle Speed",function() return ST.vehicleSpeedOn end,function() ST.vehicleSpeedOn=not ST.vehicleSpeedOn ntf("VehicleSpeed",ST.vehicleSpeedOn and "ON" or "OFF") end,"vehspeed")
...
sep(tSe)
lbl(tSe,">> ESP 2D BOX")
local tESP2D=tog(tSe,"2D Box ESP",function() return CFG.ESP2D end,function() CFG.ESP2D=not CFG.ESP2D ntf("ESP","2D Box: "..(CFG.ESP2D and "ON" or "OFF")) end,"esp2d")
table.insert(allToggles,tESP2D)
btn(tSe,"Max Distance: 2000",function() CFG.ESPMaxDist=2000 ntf("ESP","MaxDist: 2000") end,"espmd2k")
btn(tSe,"Max Distance: 5000",function() CFG.ESPMaxDist=5000 ntf("ESP","MaxDist: 5000") end,"espmd5k")
btn(tSe,"Max Distance: Unlimited",function() CFG.ESPMaxDist=99999 ntf("ESP","MaxDist: Unlimited") end,"espmdun")
sep(tSe)
lbl(tSe,">> THEME")
...
U.InputBegan:Connect(function(inp,gpe)
    ...
    for id,key in pairs(KB) do
        if inp.KeyCode==key or inp.UserInputType==key then
            ...
            elseif id=="infjump" then ST.infJump=not ST.infJump
...
R.RenderStepped:Connect(function()
    if ST.fly and LP.Character then local h=LP.Character:FindFirstChild("HumanoidRootPart") local hum=LP.Character:FindFirstChildOfClass("Humanoid") if h and hum then hum.PlatformStand=true h.Velocity=Vector3.new(0,0,0) h.RotVelocity=Vector3.new(0,0,0) local dir=Vector3.new(0,0,0) local sp=ST.flySpeed if U:IsKeyDown(Enum.KeyCode.LeftShift) then sp=sp*3 end if U:IsKeyDown(Enum.KeyCode.W) then dir=dir+CAM.CFrame.LookVector end if U:IsKeyDown(Enum.KeyCode.S) then dir=dir-CAM.CFrame.LookVector end if U:IsKeyDown(Enum.KeyCode.A) then dir=dir-CAM.CFrame.RightVector end if U:IsKeyDown(Enum.KeyCode.D) then dir=dir+CAM.CFrame.RightVector end if U:IsKeyDown(Enum.KeyCode.Space) then dir=dir+Vector3.new(0,1,0) end if U:IsKeyDown(Enum.KeyCode.LeftShift) then dir=dir-Vector3.new(0,1,0) end if dir.Magnitude>0 then dir=dir.Unit end h.CFrame=h.CFrame+dir*sp*R.RenderStepped:Wait() if ST.flyBypass then pcall(function() local bv=h:FindFirstChild("AxFlyBV") if bv then bv:Destroy() end local bg=h:FindFirstChild("AxFlyBG") if bg then bg:Destroy() end local nbv=Instance.new("BodyVelocity") nbv.Name="AxFlyBV" nbv.MaxForce=Vector3.new(math.huge,math.huge,math.huge) nbv.Velocity=Vector3.new(0,0,0) nbv.P=10000 nbv.Parent=h task.delay(0.15,function() if nbv and nbv.Parent then nbv:Destroy() end end) local nbg=Instance.new("BodyGyro") nbg.Name="AxFlyBG" nbg.MaxTorque=Vector3.new(math.huge,math.huge,math.huge) nbg.P=90000 nbg.D=100 nbg.Parent=h task.delay(0.15,function() if nbg and nbg.Parent then nbg:Destroy() end end) end) end end end
