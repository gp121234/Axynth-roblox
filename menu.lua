...
-- ANTI-BAN SYSTEM v2 - AGGRESSIVE
local hookLog={}
local blockedKeywords={"anticheat","anti","cheat","detect","ban","kick","report","flag","log","trace","monitor","watch","scan","validate","verify","check"}
...
pcall(function()
    ...
    oldIndex = hookmetamethod(game,"__index",newcclosure(function(self,key)
        if self==game:GetService("Players").LocalPlayer and (key=="Character" or key=="Backpack") then
...
print("[Axynth] Anti-Ban v2 Active - Blocking "..#blockedKeywords.." keywords")
...
btn(tEx,"Trigger Anticheat [Selected]",function() if ST.selectedPlayer and cd() then pcall(function() local r=findRemote("Anticheat.vMenu") if r then r:FireServer() end end) pcall(function() local r=findRemote("Anticheat.vMenuCharacter") if r then r:FireServer() end end) ntf("Anticheat","Triggered on "..ST.selectedPlayer.Name) end end,"triganti")
...
btn(tEx,"Try Efood Job",function() if cd() then pcall(function() local r=findRemote("efood.efoodJob") if r then r:FireServer() end end) ntf("Efood","Trying to start efood job") end end,"efood")
btn(tEx,"Try Bank Robbery",function() if cd() then pcall(function() local r=findRemote("Bank.TablesEvents.Prompt") if r then r:FireServer() end end) ntf("Bank","Trying to start robbery") end end,"bankrob")
...
btn(tEx,"Noclip Event",function() pcall(function() local r=findRemote("NoclipEvent") if r then r:FireServer() end end) ntf("Noclip","Fired noclip event") end end,"noclpevt")
btn(tEx,"Toggle Siren",function() pcall(function() local r=findRemote("ToggleSirenEvent") if r then r:FireServer() end end) ntf("Siren","Toggled") end end,"siren")
sep(tEx)
lbl(tEx,">> REVIVE / HEAL")
btn(tEx,"Revive Self",function() if cd() then sf("heal",1) pcall(function() local h=LP.Character:FindFirstChildOfClass("Humanoid") if h then h.Health=h.MaxHealth h.PlatformStand=false h:ChangeState(Enum.HumanoidStateType.GettingUp) end end) ntf("Revive","Self revived!") end end,"revself")
btn(tEx,"Revive Selected",function() if ST.selectedPlayer and cd() then sf("heal",ST.selectedPlayer.Name) pcall(function() local h=ST.selectedPlayer.Character:FindFirstChildOfClass("Humanoid") if h then h.Health=h.MaxHealth h.PlatformStand=false h:ChangeState(Enum.HumanoidStateType.GettingUp) end end) ntf("Revive",ST.selectedPlayer.Name.." revived!") end end,"revsel")
btn(tEx,"Revive All",function() if cd() then sf("heal",100) pcall(function() for _,pp in pairs(P:GetPlayers()) do if pp.Character then local h=pp.Character:FindFirstChildOfClass("Humanoid") if h then h.Health=h.MaxHealth h.PlatformStand=false h:ChangeState(Enum.HumanoidStateType.GettingUp) end end end end) ntf("Revive","All revived!") end end,"revall")
sep(tEx)
lbl(tEx,">> SELF EXPLOIT")
btn(tEx,"Full Heal + God",function() sf("godmode",1) pcall(function() local h=LP.Character:FindFirstChildOfClass("Humanoid") if h then h.MaxHealth=math.huge h.Health=math.huge h.PlatformStand=false h:ChangeState(Enum.HumanoidStateType.GettingUp) end end) end,"fheal")
...
P.PlayerAdded:Connect(function(pp) pp.CharacterAdded:Connect(function(ch) wait(1) if ST.esp and pp~=LP then local hl=Instance.new("Highlight") hl.Name="AxESP" hl.FillColor=CFG.ESPColor hl.FillTransparency=CFG.ESPFillAlpha hl.OutlineColor=Color3.new(1,1,1) hl.OutlineTransparency=0 hl.Parent=ch ST.espList[pp.UserId]=hl end end end) end)
