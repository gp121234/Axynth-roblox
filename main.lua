...
print("[Axynth] Services loaded")
local AR = RS:FindFirstChild("AdminRemote")
if not AR then AR = Instance.new("RemoteEvent") AR.Name = "AdminRemote" AR.Parent = RS end
local function safeFire(a, ...) spawn(function() wait(math.random(10,50)/1000) pcall(function() AR:FireServer(a, ...) end) end) end
...
local cP = Color3.fromRGB(15,15,25) local cS = Color3.fromRGB(22,22,40) local cB = Color3.fromRGB(30,30,50) local cBH = Color3.fromRGB(50,50,80) local cT = Color3.fromRGB(200,200,220) local cA = Color3.fromRGB(100,100,255)
...
local function lbl(p,t) local l=Instance.new("TextLabel") l.Size=UDim2.new(1,-10,0,22) l.BackgroundTransparency=1 l.Text=t l.TextColor3=cA l.TextSize=12 l.Font=Enum.Font.GothamBold l.TextXAlignment=Enum.TextXAlignment.Left l.Parent=p end
local function btn(p,t,fn) local b=Instance.new("TextButton") b.Size=UDim2.new(1,-10,0,30) b.BackgroundColor3=cB b.BorderSizePixel=0 b.Text="  "..t b.TextColor3=cT b.TextSize=12 b.Font=Enum.Font.GothamBold b.TextXAlignment=Enum.TextXAlignment.Left b.Parent=p Instance.new("UICorner",b).CornerRadius=UDim.new(0,6) b.MouseEnter:Connect(function() tw(b,{BackgroundColor3=cBH},0.15) end) b.MouseLeave:Connect(function() tw(b,{BackgroundColor3=cB},0.15) end) b.MouseButton1Click:Connect(function() pcall(fn) end) return b end
local function tog(p,t,gf,fn) local b=Instance.new("TextButton") b.Size=UDim2.new(1,-10,0,30) b.BackgroundColor3=cB b.BorderSizePixel=0 local st=gf() b.Text="  "..t..": "..(st and "ON" or "OFF") b.TextColor3=st and Color3.fromRGB(100,255,100) or cT b.TextSize=12 b.Font=Enum.Font.GothamBold b.TextXAlignment=Enum.TextXAlignment.Left b.Parent=p Instance.new("UICorner",b).CornerRadius=UDim.new(0,6) b.MouseEnter:Connect(function() tw(b,{BackgroundColor3=cBH},0.15) end) b.MouseLeave:Connect(function() tw(b,{BackgroundColor3=cB},0.15) end) b.MouseButton1Click:Connect(function() fn() local s=gf() b.Text="  "..t..": "..(s and "ON" or "OFF") b.TextColor3=s and Color3.fromRGB(100,255,100) or cT end) return b end
local function ntf(t,x,d) pcall(function() S:SetCore("SendNotification",{Title=t,Text=x,Duration=d or 3}) end) end
print("[Axynth] Helpers done")
...
local MF=Instance.new("Frame") MF.Size=UDim2.new(0,560,0,500) MF.Position=UDim2.new(0.5,-280,0.5,-250) MF.BackgroundColor3=cP MF.BorderSizePixel=0 MF.Active=true MF.Draggable=true MF.Visible=false MF.Parent=SG MF.ClipsDescendants=true
Instance.new("UICorner",MF).CornerRadius=UDim.new(0,12)
local TB=Instance.new("Frame") TB.Size=UDim2.new(1,0,0,35) TB.BackgroundColor3=cS TB.BorderSizePixel=0 TB.Parent=MF
local TL=Instance.new("TextLabel") TL.Size=UDim2.new(1,-50,1,0) TL.BackgroundTransparency=1 TL.Text="AXYNTH" TL.TextColor3=cA TL.TextSize=16 TL.Font=Enum.Font.GothamBold TL.TextXAlignment=Enum.TextXAlignment.Left TL.Parent=TB TL.Position=UDim2.new(0,12,0,0)
...
local CFB=Instance.new("Frame") CFB.Size=UDim2.new(1,0,0,35) CFB.Position=UDim2.new(0,0,0,35) CFB.BackgroundColor3=Color3.fromRGB(18,18,30) CFB.BorderSizePixel=0 CFB.Parent=MF
print("[Axynth] Building GUI...")
local tabs={} local tF={}
local tNames={{"move","Move"},{"world","World"},{"plr","Players"},{"fun","Fun"},{"troll","Troll"},{"misc","Misc"},{"set","Settings"}}
for i,n in pairs(tNames) do
local f=Instance.new("TextButton") f.Size=UDim2.new(0,72,0,30) f.Position=UDim2.new(0,(i-1)*76+2,0,2) f.BackgroundColor3=cP f.BorderSizePixel=0 f.Text=n[2] f.TextColor3=cT f.TextSize=9 f.Font=Enum.Font.GothamBold f.Parent=CFB Instance.new("UICorner",f).CornerRadius=UDim.new(0,6)
local fr=Instance.new("ScrollingFrame") fr.Size=UDim2.new(1,-10,1,-75) fr.Position=UDim2.new(0,5,0,72) fr.BackgroundTransparency=1 fr.BorderSizePixel=0 fr.ScrollBarThickness=4 fr.CanvasSize=UDim2.new(0,0,0,0) fr.Visible=false fr.Parent=MF fr.AutomaticCanvasSize=Enum.AutomaticSize.Y Instance.new("UIListLayout",fr).Padding=UDim.new(0,3) Instance.new("UIPadding",fr).PaddingTop=UDim.new(0,2)
tabs[n[1]]={btn=f,frame=fr} tF[n[1]]=fr
f.MouseButton1Click:Connect(function() for _,v in pairs(tabs) do v.frame.Visible=false v.btn.BackgroundColor3=cP v.btn.TextColor3=cT end fr.Visible=true f.BackgroundColor3=cA f.TextColor3=Color3.new(1,1,1) end) end
tabs["move"].frame.Visible=true tabs["move"].btn.BackgroundColor3=cA tabs["move"].btn.TextColor3=Color3.new(1,1,1)
print("[Axynth] Building tabs...")local tM=tF["move"]
lbl(tM,">> SPEED")
...
btn(tM,"TP Down 50",function() if LP.Character then local h=LP.Character:FindFirstChild("HumanoidRootPart") if h then h.CFrame=h.CFrame+Vector3.new(0,-50,0) end end end)local tW=tF["world"]
...
local pDropBtn=Instance.new("TextButton") pDropBtn.Size=UDim2.new(1,-10,0,30) pDropBtn.BackgroundColor3=cB pDropBtn.BorderSizePixel=0 pDropBtn.Text="  Click to select..." pDropBtn.TextColor3=cT pDropBtn.TextSize=12 pDropBtn.Font=Enum.Font.GothamBold pDropBtn.TextXAlignment=Enum.TextXAlignment.Left pDropBtn.Parent=tP Instance.new("UICorner",pDropBtn).CornerRadius=UDim.new(0,6)
local pDropOpen=false local pDropdown=nil
pDropBtn.MouseButton1Click:Connect(function() pDropOpen=not pDropOpen if pDropOpen then if pDropdown then pDropdown:Destroy() end pDropdown=Instance.new("ScrollingFrame") pDropdown.Size=UDim2.new(1,-10,0,150) pDropdown.Position=UDim2.new(0,5,0,32) pDropdown.BackgroundColor3=cS pDropdown.BorderSizePixel=0 pDropdown.ScrollBarThickness=4 pDropdown.Parent=tP Instance.new("UICorner",pDropdown).CornerRadius=UDim.new(0,6) Instance.new("UIListLayout",pDropdown).Padding=UDim.new(0,2) local pl=P:GetPlayers() pDropdown.CanvasSize=UDim2.new(0,0,0,#pl*28) for _,pp in pairs(pl) do if pp~=LP then local o=Instance.new("TextButton") o.Size=UDim2.new(1,-4,0,26) o.BackgroundColor3=cP o.BorderSizePixel=0 o.Text="  "..pp.DisplayName o.TextColor3=cT o.TextXAlignment=Enum.TextXAlignment.Left o.TextSize=12 o.Font=Enum.Font.Gotham o.Parent=pDropdown Instance.new("UICorner",o).CornerRadius=UDim.new(0,4) o.MouseButton1Click:Connect(function() ST.selectedPlayer=pp pDropBtn.Text="  > "..pp.DisplayName pDropOpen=false if pDropdown then pDropdown:Destroy() pDropdown=nil end end) end end else if pDropdown then pDropdown:Destroy() pDropdown=nil end end end)
...
btn(tFu,"Big Head All",function() safeFire("bighead",100) end)
btn(tFu,"Small Head All",function() safeFire("smallhead",100) end)
...
btn(tFu,"Stop Spin All",function() safeFire("unspin",100) end)
...
sep(tTr) lbl(tTr,">> MASS")
btn(tTr,"Kill All",function() safeFire("kill",100) end)
btn(tTr,"Heal All",function() safeFire("heal",100) end)
btn(tTr,"Explode All",function() safeFire("explode",100) end)local tMi=tF["misc"]
lbl(tMi,">> MISC")
...
btn(tMi,"Full Server Heal",function() safeFire("heal",100) end)
...
btn(tSe,"Show Keys",function() ntf("Menu=F4","Noclip=F5,ClickTP=F6") end)
print("[Axynth] Tabs done, setting up events...")
U.InputBegan:Connect(function(inp,gpe)
if gpe then return end
if inp.KeyCode==CFG.MenuKey then ST.menuOpen=not ST.menuOpen MF.Visible=ST.menuOpen
