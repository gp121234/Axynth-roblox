...
pcall(function()
    ...
    if okE and type(ET)=="table" then
        ...
        table.sort(ks)
        HOOK_APIS=HOOK_APIS.." | genv["..string.sub(table.concat(ks,","),1,3000).."]"
    end
    local XT=xnapi("Xeno")
...
local hkOk,hkErr=pcall(function()
    ...
    pcall(function()
        ...
        ST._probe=ST._probe..string.format("|tp=%.1f pool=%d ms=%d cli=%d",tick()-tC,#pool,skipped,#clis)
        local tR=tick()
        local gi=xnapi("getinstances")
        if type(gi)=="function" then
            local okR,insts=pcall(gi)
                ...
                for _,inst in pairs(insts) do
                    ...
                    if cn=="RemoteEvent" or cn=="RemoteFunction" or cn=="UnreliableRemoteEvent" then
                        ...
                        elseif fnm:find(".Cars.Cars.",1,true) then
                            cjN=cjN+1
                        elseif fnm:find("HDAdminHDClient.Signals.",1,true) then
                            hdN=hdN+1
                        else
        ...
        for _,i in ipairs(pool) do
            if #fetch>=45 then break end
            fetch[#fetch+1]=i
            inF[i]=true
        end
        for _,i in ipairs(clis) do
            if #fetch>=60 then break end
            if not inF[i] then
                fetch[#fetch+1]=i
                inF[i]=true
            end
        ...
        ST._probe=ST._probe..string.format("|tb=%.1f bc=%d/e%d",tick()-tB,bok,bfail)
        if #khidx>0 then
            local t2={}
            ...
            pcall(function()
                ...
                for _,inst in ipairs(game:GetDescendants()) do
                    ...
                    if cn=="RemoteEvent" or cn=="UnreliableRemoteEvent" then
                        ...
                        local fnm=(okf and f or ""):lower()
                        local hit=false
                        for _,kw in ipairs({"job","society","shop","market","super","invent","armory","team","car","dealer","garage","vehicle","weapon","claim","crate","bank","drop"}) do
                            if fnm:find(kw,1,true) then hit=true break end
                        end
                        if hit and not fnm:find("materialservice",1,true) and not fnm:find("hdadmin",1,true) then
                            local okc=false
                            pcall(function()
    ...
    elseif p1ok then
        ...
        local pHIok,pHIerr=false,"dead(inst-swap,no-cb)"
        local pHIdone=true
        local _skipHI=true
        if not _skipHI then
        task.spawn(function()
        ...
        if _skipHI then error("pHI["..pHIerr.."] path4:dead(hookfunction-noop)",0) end
...
local function axPromptInteract(kws, clickName, dlSec)
    ...
    local function scanUI(bw)
        pcall(function()
            ...
            local function mine(d)
                ...
                local nm=string.lower(tostring(sg.Name or ""))
                if nm:find("axynth",1,true) or nm:find("remotescanner",1,true) or nm:find("remotespy",1,true) or nm:find("axpalette",1,true) then return false end
                return true
            end
            local function hit(blob)
                blob=string.lower(blob)
                if blob:find(bw,1,true) then return true end
                local all=true
                local cnt=0
            ...
            local function actionish(b)
                local t=string.gsub(string.lower(tostring(b.Text or "").." "..tostring(b.Name or "")),"[^%w]","")
                for _,k in ipairs({"purchase","buy","applyjob","apply","select","join","choose","hire","confirm","accept","equip","take","add","ok","yes","spawn","spawnvehicle","buyvehicle","claim","redeem","continue","get"}) do
                    if t:find(k,1,true) then return true end
                end
                return false
            end
            local function btnOf(lbl)
                local scope=lbl.Parent
                for _=1,3 do
                    if not scope then return nil end
                    if (scope:IsA("TextButton") or scope:IsA("ImageButton")) and actionish(scope) then return scope end
                    if scope:IsA("Frame") or scope:IsA("ScrollingFrame") or scope:IsA("CanvasGroup") then
                        local fb=nil
                        for _,bd in ipairs(scope:GetDescendants()) do
                            if (bd:IsA("TextButton") or bd:IsA("ImageButton")) and actionish(bd) and mine(bd) then fb=bd break end
                        end
            ...
            for pass=1,2 do
                for _,d in ipairs(g:GetDescendants()) do
                    local cn=d.ClassName
                    if pass==1 and (cn=="TextButton" or cn=="ImageButton") and mine(d) then
                        ...
                        if hit(blob) then found=d break end
                    elseif pass==2 and cn=="TextLabel" and mine(d) then
                        local txt=tostring(d.Text or "").." "..tostring(d.Name or "")
                        if hit(txt) then
                            local fb=btnOf(d)
                            if fb then found=fb break end
                        end
                    end
                end
                if found then break end
    ...
    if n>0 then
        ...
        pcall(function()
            local primary=found
            task.wait(0.6)
            for _,cfw in ipairs({"applyjob","confirm","accept","purchase","hire","buy"}) do
                found=nil
                scanUI(cfw)
                local cf=found
                if cf and cf~=primary then
                    local bad=false
                    local okp,cfp=pcall(function() return cf:GetFullName() end)
                    local cfl=string.lower((okp and cfp) or "")
                    if cfl:find("hdadmin",1,true) or cfl:find("vmenu",1,true) or cfl:find("anticheat",1,true) then bad=true end
                    if (cfw=="buy" or cfw=="purchase") and not bad then
                        local p1=cf:FindFirstAncestorOfClass("ScreenGui")
                        local p2=primary and primary:FindFirstAncestorOfClass("ScreenGui")
                        if p1 and p2 and p1~=p2 then bad=true end
                    end
                    if not bad then
                        local n2=0
                        pcall(function()
                            for _,c in pairs(getconnections(cf.MouseButton1Click)) do
                                pcall(function() c.Function(cf) n2=n2+1 end)
                            end
                        end)
                        if n2==0 then
                            pcall(function()
                                for _,c in pairs(getconnections(cf.Activated)) do
                                    pcall(function() c.Function(cf) n2=n2+1 end)
                                end
                            end)
                        end
                        if n2>0 then
                            ntf("Shop","Confirm: "..cfw.." clicked",5)
                        end
                        break
                    end
                end
